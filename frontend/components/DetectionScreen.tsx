...
export function DetectionScreen({ initialText, onAccepted, onEdit }: { initialText?: string; onAccepted: (app: Application) => void; onEdit: (draft: ApplicationDraft) => void }) {
  ...
  return (
    <ScrollView style={styles.wrap} contentContainerStyle={styles.content}>
      ...
      <View style={styles.sourceRow}>{(['share', 'screenshot', 'email'] as Source[]).map((s) => <Pressable key={s} onPress={() => setSource(s)} style={[styles.source, source === s && styles.sourceActive]} {...webTest(`source-${s}`)}><Text style={[styles.sourceText, source === s && styles.sourceTextActive]}>{s.toUpperCase()}</Text></Pressable>)}</View>
      ...
      <TextInput multiline value={text} onChangeText={setText} placeholder="Paste shared job text, email snippet, or OCR text..." placeholderTextColor="#71717A" style={styles.textarea} {...webTest('detection-text-input')} />
