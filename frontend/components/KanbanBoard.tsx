  ...
  return (
    <ScrollView style={styles.wrap} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.summaryRow}>
        {STATUSES.map((status) => <StatusSummary key={status} status={status} selected={selected === status} count={applications.filter((a) => a.status === status).length} width={cardWidth} onPress={() => setSelected(status)} />)}
      </ScrollView>
      ...
      <View style={styles.listCard} testID={`kanban-column-${selected.toLowerCase()}`} {...webTest(`kanban-column-${selected.toLowerCase()}`)}>
        {visibleApps.length === 0 ? <EmptyState selected={selected} /> : visibleApps.map((card, index) => <JobRow key={card.id} app={card} isLast={index === visibleApps.length - 1} onStatusChange={onStatusChange} />)}
      </View>
...
function StatusSummary({ status, count, width, selected, onPress }: { status: Status; count: number; width: number; selected: boolean; onPress: () => void }) {
...
const styles = StyleSheet.create({
  ...
  summaryRow: { paddingHorizontal: 20, gap: 12, paddingVertical: 10 }, statusCard: { height: 126, borderRadius: 22, backgroundColor: '#11111A', borderWidth: 1, borderColor: '#242432', alignItems: 'center', justifyContent: 'center', padding: 12, gap: 7 }, statusCardActive: { borderColor: '#5B6578', backgroundColor: '#151520' }, statusIcon: { width: 48, height: 40, borderRadius: 18, alignItems: 'center', justifyContent: 'center' }, statusLabel: { color: '#FFFFFF', fontSize: 14, fontWeight: '900' }, statusCount: { color: '#FFFFFF', fontSize: 29, lineHeight: 32, fontWeight: '900' }, statusLine: { position: 'absolute', bottom: 11, left: 20, right: 20, height: 4, borderRadius: 99 },
